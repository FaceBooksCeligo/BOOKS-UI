...
export default function NewSalesOrderPage() {
  ...
  const calculateItemTotals = (item: Partial<OrderItem>) => {
  ...
  const calculateOrderTotals = () => {
    const subtotal = orderItems.reduce((sum, item) => sum + item.extendedAmount, 0);
  ...
  const handleUpdateItem = () => {
    ...
    setOrderItems(prev => prev.map(item => item.id === editingItem.id ? updatedItem : item));
    resetItemForm();
    setEditingItem(null);
    setShowItemForm(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("Sales order saved as draft");
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => handleInputChange("fulfillmentDate", e.target.value)}
                  />
                </div>
              </div>
