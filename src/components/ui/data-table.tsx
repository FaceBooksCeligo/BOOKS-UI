...
export function createStatusColumn<TData>(
  ...
  statusMap: Record<string, { label: string; variant: "default" | "destructive" | "outline" | "secondary" | "ghost" | "link" | "success" | "warning" | "info" }>
): ColumnDef<TData> {
  return {
...
export function createMoneyColumn<TData>(
  ...
  return {
    ...
    header,
    cell: ({ row }) => {
      const money = row.getValue(accessorKey as string) as { txn: string; base: string };
      const amount = parseFloat(money?.txn || "0");
      return (
...
export function createDateColumn<TData>(
  ...
  return {
    ...
    header,
    cell: ({ row }) => {
      const date = row.getValue(accessorKey as string) as string;
      return (
        <div>
